...
// استخدام نفس مفاتيح localStorage
const STORAGE_KEYS = {
    RESERVATIONS: 'dental_reservations',
    MANH_CASES: 'dental_manh_cases',
    CANCELLED: 'dental_cancelled_cases'
};
...
const reverseCourseMapping = {
    'restorative': 'restorative4',
    'exodontia': 'exodontia4',
    'periodontics': 'periodontics2',
    'endodontics': 'endodontics4'
};
...
// تحويل حالة الحجز إلى حالة الحالة
function mapStatus(reservationStatus, type) {
    if (type === 'manh') return 'transferred'; // المنح تعتبر محولة
    switch(reservationStatus) {
        case 'reserved': return 'waitingAcceptance';
...
// ========== بيانات الحالات - الآن تُحمل من نفس مصدر الكود الثاني ==========
let casesData = [];

// تحويل حجز إلى حالة (للعرض في الجدول)
function reservationToCase(reservation, index) {
    const courseId = courseMapping[reservation.courseId] || 'restorative';
    const courseNames = {
        'restorative': 'مداواة الأسنان الترميمية 4',
        'exodontia': 'تخدير و قلع الأسنان 4',
        'periodontics': 'النسج حول سنية 2',
        'endodontics': 'مداواة الأسنان اللبية 4'
    };
    
    // البحث عن معلومات المريض من الكود الثاني إن وجدت
    let patientName = `مريض #${reservation.patientId}`;
    let supervisor = 'د. المشرف';
    
    // إذا كان هناك بيانات المريض في الكود الثاني
    if (typeof mockPatients !== 'undefined') {
        const patient = mockPatients.find(p => p.id === reservation.patientId);
        if (patient) {
    ...
    return {
        id: 1000 + index, // معرف فريق للعرض
        reservationId: reservation.patientId + '_' + reservation.toothNumber, // مرجع للحجز الأصلي
        patientName: patientName,
        caseType: getCaseTypeFromTreatment(reservation.treatmentValue),
        course: courseNames[courseId],
        courseId: courseId,
        status: mapStatus(reservation.status, reservation.type),
        ratings: 0,
        ratingDates: [null, null, null],
        supervisor: supervisor,
        addedDate: reservation.date,
        acceptedDate: reservation.status === 'accepted' ? reservation.date : null,
        requiresImages: reservation.status === 'accepted' || reservation.status === 'inProgress',
        requiresPanorama: false,
        toothNumber: reservation.toothNumber,
        treatmentValue: reservation.treatmentValue,
        originalReservation: reservation // الاحتفاظ بالمرجع
    };
}

// تحميل الحالات من نفس مصدر الكود الثاني
function loadCasesFromSharedStorage() {
    try {
        ...
        if (savedReservations) {
            ...
            // تحويل الحجوزات إلى حالات للعرض
            casesData = reservations
                .filter(r => !['deleted', 'cancelled'].includes(r.status))
                .map((r, i) => reservationToCase(r, i));
        }
        
        // تحميل بيانات الاستديو (تبقى منفصلة)
        const savedStudio = localStorage.getItem('dentalStudioData_v2');
        if (savedStudio) {
...
// حفظ الاستديو فقط (الحالات تُحفظ عبر الكود الثاني)
...
// ... (جميع دوال الـ Modal والـ Studio والـ Upload تبقى unchanged)

// ========== Event Listeners المحدثة ==========
document.addEventListener('DOMContentLoaded', function() {
    ...
    // الاستماع لتغييرات الكود الثاني
    window.addEventListener('storage', function(e) {
        if (e.key === STORAGE_KEYS.RESERVATIONS || e.key === STORAGE_KEYS.MANH_CASES) {
            loadCasesFromSharedStorage();
            loadCasesTable();
        }
    });
...
// ========== دوال placeholder للدوال المفقودة ==========
