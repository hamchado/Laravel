...
<body>
            ...
            <form id="loginForm">
                <div class="form-group">
                    ...
                    <div class="input-wrapper">
                        <input type="text" class="form-input" id="student_id" placeholder="أدخل الرقم الجامعي" required autofocus>
            ...
            <div class="modal-body">
                ...
                <div class="modal-step" id="step2">
                    <div class="modal-info-box warning">
                        ...
                        <p>تم إرسال رمز التحقق. تواصل مع المشرف للحصول على الرمز وأدخله هنا.</p>
                    </div>
    ...
    <script>
        ...
        document.getElementById('loginForm').addEventListener('submit', function(e) {
            ...
            apiFetch('/login', { student_id: studentId, password: password })
                .then(data => {
                    if (data.success) {
                        ...
                        btn.disabled = false;
                        showError(data.message || 'الرقم الجامعي أو كلمة المرور غير صحيحة');
                    }
                })
                .catch(() => {
                    btn.classList.remove('loading');
        ...
        document.getElementById('requestOtpForm').addEventListener('submit', function(e) {
            ...
            apiFetch('/auth/request-otp', { student_id: studentId })
                .then(data => {
                    btn.classList.remove('loading');
                    btn.disabled = false;
                    if (data.success) {
                        ...
                        showStepMessage(1, data.message || 'حدث خطأ', 'error');
                    }
                })
                .catch(() => {
                    btn.classList.remove('loading');
                    btn.disabled = false;
                    showStepMessage(1, 'حدث خطأ في الاتصال', 'error');
                });
        });
        ...
        function verifyOtp() {
            ...
            apiFetch('/auth/verify-otp', { student_id: savedStudentId, code: otp })
                .then(data => {
                    btn.classList.remove('loading');
                    if (data.success) {
                        ...
                        btn.disabled = false;
                        showStepMessage(2, data.message || 'رمز التحقق غير صحيح', 'error');
                        otpInputs.forEach(i => i.classList.add('error'));
                    }
                })
                .catch(() => {
                    btn.classList.remove('loading');
                    btn.disabled = false;
                    showStepMessage(2, 'حدث خطأ في الاتصال', 'error');
                });
        }
        ...
        document.getElementById('newPasswordForm').addEventListener('submit', function(e) {
            ...
            .then(data => {
                ...
                if (data.success) {
                    ...
                    btn.disabled = false;
                    showStepMessage(3, data.message || 'حدث خطأ', 'error');
                }
            })
            .catch(() => {
                btn.classList.remove('loading');
                btn.disabled = false;
                showStepMessage(3, 'حدث خطأ في الاتصال', 'error');
            });
        });
